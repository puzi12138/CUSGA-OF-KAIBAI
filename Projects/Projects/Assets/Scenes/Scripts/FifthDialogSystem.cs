...
//第五章对话框系统
public class FifthDialogSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space) || Input.GetMouseButtonDown(0))
        {
            if (textFinished && !cancelTyping)//若文件正在输出且未取消输出则继续执行
            {
                ...
                if (index == 1)
                {
                    GameObject.Find("Dialogs/Canvas01/Panel/MainPlayer").SetActive(true);//在第一行文字打出时，使得人物介绍显示
                    GameObject.Find("Dialogs/Canvas01/Panel/Player02").SetActive(true);//在第一行文字打出时，使得人物图片显示
                }
                if (index == 2)
                {
                    GameObject.Find("Dialogs/Canvas01/Panel/MainPlayer").SetActive(false);//在第二行文字打出后，使得人物介绍关闭
                }
                if(index==13)
                {
                    anim.SetTrigger("Player02");//在第13行文字打出时，执行动画
                }
            }
